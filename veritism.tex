...
\begin{itemize}
    ...
    see \textcite{Sosa2007-SOSAVE-2} for fundamental vs derivative value.
    \item{ Good vs. right }
...
\subsection{How Can Veritism Ground Epistemic Normativity?}
...
Let's compare these two beliefs:
\begin{enumerate}
...
Intuitively, truly believing (1) seems a lot more valuable than believing (2). \textcite{sep-knowledge-value} put it like this:
...
Why is it that some beliefs seem more valuable than others? I see crucial differences along two dimensions here.
First, the significance of the belief. For most people, believing a proposition is valuable only if it is of interest, be it for day-to-day life or for other purposes. Hence, concerning oneself for no good reason remembering each entry in a phone book seems like a utterly point- and valueless undertaking, even though this, add lots of true beliefs to ones overall stock. An important distinction should be made here between subjective and objective significance. A belief could be more or less relevant to one's own purposes. In that sense, my own interest determines what is significant to me. Whether the number of people ever to have visited the David Hume memorial up until now is even or odd may not be of interest to most people, but if bet running maybe! epistmic value should not be relative to subjective interests, though.
Secondly, the specificity of the belief.


Some beliefs are more intuitively more valuable than others. 

Differentiate between subjective and objective significance? is it significant just for S or is it significant in an objective sense?

if subjective, then the the epistemically valuable would depend on what is significant for S. S may be completely lunatic. That's counterintuitive.

if objective, there needs to be an explanatory story as to how.

\textcite{Goldman2002-GOLTUO-2} answer: Add significance to the epistemically valuable, so that only significant true beliefs can be of value

\textcite{Berker2013-BERETA-2}: Just state that not all true beliefs are valuable, but that \emph{only} true beliefs are valuable.


\autocite{sep-knowledge-value}
